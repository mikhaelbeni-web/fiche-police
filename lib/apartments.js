...
// Résout TOUTES les sous-unités physiques d'une réservation. La plupart des
// réservations n'en ont qu'une, mais une réservation peut légitimement couvrir
// plusieurs appartements à la fois (ex. groupe de voyageurs sur chambres
// communicantes) : rv.reservationUnit contient alors plusieurs entrées.
// L'ancienne logique ne lisait que reservationUnit[0], perdant silencieusement
// les autres appartements de la réservation (arrivées/départs/linge/taxes).
// Retourne toujours un tableau, jamais null (vide si rien de résoluble).
export function resolveApartments(rv, listingId) {
  const ru = rv.reservationUnit;
  if (Array.isArray(ru) && ru.length > 0) {
    const matched = ru.map(u => APARTMENTS[String(u.listingUnitId)]).filter(Boolean);
    if (matched.length > 0) return matched;
  }
  const single = resolveApartment(rv, listingId);
...
// Un ménage "décalé" déplace le ménage du jour du check-out (datePrevue) vers
// une autre date (date). Le départ d'origine ne doit donc plus apparaître ni
// être facturé ce jour-là : il a été déplacé, pas dupliqué.
// Ne concerne que les décalés portant le marqueur movesDeparture (les anciens,
// enregistrés à 0 €, gardent leur comportement d'origine tant qu'ils n'ont pas
// été repris via le bouton dédié sur /linge).
export function isDepartureMoved(item, extraMenages) {
  if (!item || item.extra) return false;
  const day = (item.depart || "").slice(0, 10);
  if (!day) return false;
  return (extraMenages || []).some(e =>
    e.type === "decale" &&
    e.movesDeparture === true &&
    (e.datePrevue || "").slice(0, 10) === day &&
    (e.unitNumber || "") === (item.unitNumber || "") &&
    (e.appartement || "") === (item.appartement || "")
  );
}

// No-show : ménage masqué manuellement (client jamais arrivé), via /linge.
// Doit exclure la ligne PARTOUT où le ménage est compté — planning (menage.js),
// coûts (couts.js) et feuille de linge (linge.js) — sinon le coût resterait
// facturé alors que le ménage n'a jamais eu lieu. Clé : listingId si dispo
// (départs réels), sinon unitNumber (ménages supplémentaires sans listingId).
export function isNoShowHidden(item, hiddenMenages) {
  if (!item || !hiddenMenages || hiddenMenages.length === 0) return false;
  const day = (item.depart || item.date || "").slice(0, 10);
  if (!day) return false;
  return hiddenMenages.some(h => {
    if ((h.date || "").slice(0, 10) !== day) return false;
    if (h.listingId) return String(h.listingId) === String(item.listingId);
    return (h.unitNumber || "") === (item.unitNumber || "");
  });
}
